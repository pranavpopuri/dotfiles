...
; ── Local overrides (appended last so they win) ────────────────
; Pointer / reference punctuation in types → red, matching the VSCode theme.
; Upstream tags these as @type.builtin (cyan); recapture as @operator (red).
(pointer_declarator "*" @operator)
(abstract_pointer_declarator "*" @operator)
(reference_declarator ["&" "&&"] @operator)
(abstract_reference_declarator ["&" "&&"] @operator)

; Semicolons → red (upstream groups them under @punctuation.delimiter = gray)
";" @operator
